...
class ManagementConsole(tk.Tk):
    def __init__(self, global_state: GlobalState):
        ...
        self.blacklist_frame = ttk.Frame()
        self.console_frame = ttk.Frame()

        # Create URL input field and add button
        self.url_input = ttk.Entry(master=self.blacklist_frame, width=40)
        self.url_input.grid(row=0, column=0, padx=10, pady=10)

        self.add_button = ttk.Button(master=self.blacklist_frame, text="Add to Blacklist", command=self.add_to_blacklist)
        self.add_button.grid(row=0, column=1, padx=10, pady=10)

        # Create label for blacklisted URLs
        self.blacklist_label = ttk.Label(master=self.blacklist_frame, text="Blacklisted URLs:")
        ...
        self.delete_button = ttk.Button(master=self.blacklist_frame, text="Remove 1 from Blacklist", command=self.delete_1_blacklist)
        ...
        self.cache_checkbox = ttk.Checkbutton(master=self.blacklist_frame, text="Enable Caching", variable=self.cache_var, command=self.toggle_cache)
        self.cache_checkbox.grid(row=4, pady=5)

        # Create textfields and labels for the 2 display areas
        self.output_text_label = ttk.Label(master=self.console_frame, text="Byte Flow:", font=("Arial", 12))
        ...
        self.connections_text_label = ttk.Label(master=self.console_frame, text="Connection Flow:", font=("Arial", 12))
